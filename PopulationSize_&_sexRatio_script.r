estim_betaks<-read.csv2('PopulationSize_Betas_Konjsko.csv',h=T)  #EXTRACT BETAS OF CAPTURE PROBABILITIES FOR KONJSKO
varcovks<-read.csv2('PopulationSize_variancecovarianceMatrix_Konjsko.csv',h=F)   #EXTRACT VARIANCE COVARIANCE MATRICES OF CAPTURE PROBABILITIES FOR KONJSKO

estim_beta<-read.csv2('PopulationSize_Betas_GolemGrad.csv',h=T)	 #EXTRACT BETAS OF CAPTURE PROBABILITIES FOR GOLEM GRAD ISLAND
varcov<-read.csv2('PopulationSize_variancecovarianceMatrix_GolemGrad.csv',h=F)   #EXTRACT VARIANCE COVARIANCE MATRICES OF CAPTURE PROBABILITIES FOR GOLEM GRAD ISLAND   

numbetaks<-c(4:18) #identify the capture probability betas for Konjsko
...
numbeta<-c(9:188) #identify the capture probability betas for Golem Grad
...
# Create matrices to calculate population sizes, adult sex ratios and operational sex ratios on the Beach and Plateau of Golem Grad Island and in Konjsko village

fp09<-matrix(NA,nrow=n,ncol=1)    #Plateau females in 2009
...
fp23<-matrix(NA,nrow=n,ncol=1)	   #Plateau females in 2023

fb09<-matrix(NA,nrow=n,ncol=1)	   #Beach females in 2009
...
fb23<-matrix(NA,nrow=n,ncol=1)	   #Beach females in 2023

ofp09<-matrix(NA,nrow=n,ncol=1)	   #Reproductively active Plateau females in 2009
...
ofp23<-matrix(NA,nrow=n,ncol=1)	   #Reproductively active Plateau females in 2023

ofb09<-matrix(NA,nrow=n,ncol=1)	   #Reproductively active Beach females in 2009
...
ofb23<-matrix(NA,nrow=n,ncol=1)	   #Reproductively active Beach females in 2023

mp09<-matrix(NA,nrow=n,ncol=1)	   #Plateau males in 2009
...
mp23<-matrix(NA,nrow=n,ncol=1)	   #Plateau males in 2023

mb09<-matrix(NA,nrow=n,ncol=1)	   #Beach males in 2009
...
mb23<-matrix(NA,nrow=n,ncol=1)	   #Beach males in 2023

psr09<-matrix(NA,nrow=n,ncol=1)	   #Adult sex ratio on the Plateau in 2009
...
psr23<-matrix(NA,nrow=n,ncol=1)	   #Adult sex ratio on the Plateau in 2023

bsr09<-matrix(NA,nrow=n,ncol=1)	   #Adult sex ratio on the Beach in 2009
...
bsr23<-matrix(NA,nrow=n,ncol=1)	   #Adult sex ratio on the Beach in 2023

posr09<-matrix(NA,nrow=n,ncol=1)	   #Operational sex ratio on the Plateau in 2009
...
posr23<-matrix(NA,nrow=n,ncol=1)	   #Operational sex ratio on the Plateau in 2023

bosr09<-matrix(NA,nrow=n,ncol=1)	   #Operational sex ratio on the Plateau in 2009
...
bosr23<-matrix(NA,nrow=n,ncol=1)	   #Operational sex ratio on the Plateau in 2023


mks11<-matrix(NA,nrow=n,ncol=1)	   #Males in Konjsko in 2009
...
mks23<-matrix(NA,nrow=n,ncol=1)	   #Males in Konjsko in 2023

fks11<-matrix(NA,nrow=n,ncol=1)	   #Females in Konjsko in 2009
...
fks23<-matrix(NA,nrow=n,ncol=1)	   #Females in Konjsko in 2023

ofks11<-matrix(NA,nrow=n,ncol=1)	   #Reproductively active females in Konjsko in 2009
...
ofks23<-matrix(NA,nrow=n,ncol=1)	   #Reproductively active females in Konjsko in 2023

srks11<-matrix(NA,nrow=n,ncol=1)	   #Adult sex ratio in Konjsko in 2009
...
srks23<-matrix(NA,nrow=n,ncol=1)	   #Adult sex ratio in Konjsko in 2023

osrks11<-matrix(NA,nrow=n,ncol=1)	   #Operational sex ratio in Konjsko in 2009
...
osrks23<-matrix(NA,nrow=n,ncol=1)	   #Operational sex ratio in Konjsko in 2009


#CALCULATE POPULATION SIZES WITH 95% CONFIDENCE INTERVALS BY DIVIDING RAW NUMBERS BY RESPECTIVE BETAS OF CAPTURE PROBABILITIES, AND FURTHER BY MULTIPLYING BY PROPORTIONS OF REPRODUCTIVELY ACTIVE FEMALES PER LOCALITY TO OBTAIN NUMBERS OF BREEDING FEMALE POPULATIONS SIZES AND OPERATIONAL SEX RATIOS
...
#CALCULATE SEX RATIOS FROM MATRICES
 ...
 #### ANOVA to compare means of sex ratios per year 
data <- data.frame(
  one = psr23,
  two = bsr23,
  three = srks23
)
#sort the data as needed (package 'tidyr')
data_long <- tidyr::pivot_longer(data, cols = c(one, two, three), 
...
# get comparisons between each group sepparately
tukeyR<-TukeyHSD(anova_result)
write.csv((as.data.frame(tukeyR$group)), file = "tukey23.csv", row.names = TRUE)
